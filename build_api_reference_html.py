...
def render_table(lines):
    ...
    for index, line in enumerate(lines):
        ...
        rendered = "".join("<%s>%s</%s>" % (tag, inline(cell), tag) for cell in cells)
    ...
    while index < len(lines):
        ...
        if heading:
            ...
            if level == 1:
                ...
                output.append('<h3 id="%s">%s</h3>' % (anchor, inline(text)))
            index += 1
            continue
...
{content}<p id="empty" class="empty">No matching API entries.</p></main></div><script>{JS}</script></body></html>"""
